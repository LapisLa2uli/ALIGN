...
class Dataset(Dataset):
    ...
    def __init__(
        ...
        if self.validation_windows < 1 or (self.validation_windows > 1 and not (full_recording and is_deterministic)):
            raise ValueError("Multiple validation windows require deterministic full-recording sampling")
        self._validation_fraction = 0.0
    ...
    def _random_chunk(self, row):
        ...
        if random_length < 1:
            ...
            start_length = (
                random.randint(0, random_length) if not self.is_deterministic else int(round(self._validation_fraction * random_length))
            )
            padding_length = 0
    ...
    def __getitem__(self, idx):
        slot = idx % self.validation_windows
        idx = idx // self.validation_windows
        self._validation_fraction = slot / max(1, self.validation_windows - 1)
        ...
        rows = [row] if self.full_recording else self._split_frame(row, length=self.split_frame_length)
    ...
    def __len__(self):
        return len(self.df) * self.validation_windows
