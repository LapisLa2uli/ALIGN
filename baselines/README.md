...
The upstream generator at the base revision above produces `note_map.json`, whose semantics differ from `note_labels.json`. In particular, it does not directly provide complete missing-note timings on the performance timeline. The converter explicitly rejects this input, **rather than silently dropping data or inventing supervision labels**. Before generating a new training dataset, implement and validate label export for the new schema. This is unnecessary when using the existing frozen data.
