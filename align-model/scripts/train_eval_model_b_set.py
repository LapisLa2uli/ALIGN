"""Train Model B set-F1 checkpoints (new dirs) and score exclusive set matching."""
...
LOG_PATH = PRED_DIR / "train_model_b_set.log"
SUMMARY_PATH = PRED_DIR / "summary_model_b_set.json"

FORBIDDEN_OUT = {
    RUNS / "melody-random12k",
    RUNS / "melody-raw2k",
}
...
JOBS = [
    {
        "id": "B-random12k-set",
        "data": RANDOM_ROOT,
        "out": RUNS / "melody-random12k-set",
    },
    {
        "id": "B-raw2k-set",
        "data": RAW_ROOT,
        "out": RUNS / "melody-raw2k-set",
    },
]
...
def train_job(job: dict, epochs: int, device: str) -> dict:
    ...
    data = Path(job["data"])
    if out in {p.resolve() for p in FORBIDDEN_OUT}:
        raise RuntimeError(f"refusing to overwrite legacy checkpoint dir {out}")
...
def evaluate_b(n_eval: int, device: str) -> dict:
    ...
    report: dict = {"n_eval": n_eval, "device": device, "metric": "set-F1", "cells": []}
    ...
    for job in JOBS:
        ...
        for ev in EVAL_SETS:
            ...
            log(
                ...
                f"n_pred={cell['mean_n_pred']:.2f} n_gold={cell['mean_n_gold']:.2f} "
...
def main() -> None:
    ...
    log(
        f"model-B set-F1 train/eval random={summary['n_random']} raw={summary['n_raw']} "
        f"epochs={args.epochs} n_eval={args.n_eval}"
    )
    if not args.skip_train:
        for job in JOBS:
            try:
                summary["train_runs"].append(train_job(job, epochs=args.epochs, device=args.device))
    ...
    if not args.skip_eval and ready:
        try:
            summary["eval"] = evaluate_b(args.n_eval, args.device)
        except Exception as exc:
        ...
        log("SKIP eval (not both B set checkpoints present)")
